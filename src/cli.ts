...
Commands:
  ...
  wordmanner context --language en|ru --medium <medium> [--audience <text>] [--intent <text>] [--topic <text>] [--store <path>]
...
async function run(args: string[]): Promise<void> {
  ...
  if (command === "context") {
    ...
    console.log(renderContext(query, selectExamples(await loadSamples(store), query)));
    return;
  }
  if (command === "bench") {
    const action = args.shift();
    if (action === "blind") {
      ...
      console.log(`Prepared ${count} blind cases in ${output}. Give raters ballot.jsonl only; keep key.json private.`);
      return;
    }
    if (action === "score") {
